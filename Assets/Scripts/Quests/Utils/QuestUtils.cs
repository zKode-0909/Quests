...
public static class QuestUtils
{
    public static QuestDisplayIcon DetermineIcon(QuestGiver questGiver,QuestLog questLog,int level) {
        ...
        foreach (var quest in questGiver.Quests) {
            if (quest.RequiredLevel <= level) {
                ...
                {
                    if (questLog.quests[quest.ID].GetQuestCompletionStatus())
                    {
                        displayIcons.Add(QuestDisplayIcon.Completed);
                    }
                    else {
        ...
        if (displayIcons.Contains(QuestDisplayIcon.Completed))
        {
            return QuestDisplayIcon.Completed;
        }
        else if (displayIcons.Contains(QuestDisplayIcon.Available))
        {
    ...
    public static QuestStatus DetermineQuestStatus(QuestSettings quest,QuestLog questLog,int level) {
        if (questLog.GetQuests().ContainsKey(quest.ID)) {
            if (questLog.GetQuests()[quest.ID].GetQuestCompletionStatus())
            {
    ...
    public static QuestStatus DetermineQuestStatus(Quest quest)
    {
        if (quest.GetQuestCompletionStatus())
        {
            return QuestStatus.Complete;
        }
        else {
